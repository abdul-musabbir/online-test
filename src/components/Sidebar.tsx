...
const Sidebar: React.FC<SidebarProps> = ({ isOpen }) => {
  ...
  return (
    <div className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-blue-900 to-blue-800 text-white transition-all duration-300 z-20
      ${isOpen ? 'w-64' : 'w-0 md:w-20'}`}>
      <div className={`h-full ${isOpen ? 'opacity-100' : 'opacity-0 md:opacity-100'} transition-opacity duration-300`}>
        <div className="flex items-center justify-center h-16 mb-8">
          <GraduationCap size={24} className="mr-2" />
          <h1 className={`text-xl font-bold transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'hidden md:hidden'}`}>
            ExamPro
          </h1>
        </div>
